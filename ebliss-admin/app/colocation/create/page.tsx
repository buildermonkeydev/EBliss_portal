...
import { ArrowLeft, Save, Loader2, Building2, Zap, Wifi, HardDrive, Shield, Plus, Trash2, ChevronDown, ChevronUp } from 'lucide-react';
import { LayoutWrapper } from '../../components/layout/LayoutWrapper';
import { createColocation, getAvailableRacks, Rack } from '../../lib/colocation';
...
export default function CreateColocationPage() {
  ...
  const [showBgpConfig, setShowBgpConfig] = useState(false);
  ...
  const [bgpConfig, setBgpConfig] = useState<BgpConfig>({
    enabled: false,
    asn: '',
    ipv4_prefixes: [],
    ipv6_prefixes: [],
  ...
  const [ipv4PrefixInput, setIpv4PrefixInput] = useState('');
  const [ipv6PrefixInput, setIpv6PrefixInput] = useState('');

  const [powerFeeds, setPowerFeeds] = useState<PowerFeed[]>([
    { name: 'Feed A', voltage: 230, amperage: 16, phase: 'single' }
  ]);
...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      setPops(popsResponse.filter((p: Pop) => p.active));
    } catch (error) {
  ...
  const addIpv4Prefix = () => {
    if (ipv4PrefixInput.trim() && !bgpConfig.ipv4_prefixes.includes(ipv4PrefixInput.trim())) {
      setBgpConfig({
        ...bgpConfig,
        ipv4_prefixes: [...bgpConfig.ipv4_prefixes, ipv4PrefixInput.trim()]
      });
      setIpv4PrefixInput('');
    }
  };

  const removeIpv4Prefix = (prefix: string) => {
    setBgpConfig({
      ...bgpConfig,
      ipv4_prefixes: bgpConfig.ipv4_prefixes.filter(p => p !== prefix)
    });
  };

  const addIpv6Prefix = () => {
    if (ipv6PrefixInput.trim() && !bgpConfig.ipv6_prefixes.includes(ipv6PrefixInput.trim())) {
      setBgpConfig({
        ...bgpConfig,
        ipv6_prefixes: [...bgpConfig.ipv6_prefixes, ipv6PrefixInput.trim()]
      });
      setIpv6PrefixInput('');
    }
  };

  const removeIpv6Prefix = (prefix: string) => {
    setBgpConfig({
      ...bgpConfig,
      ipv6_prefixes: bgpConfig.ipv6_prefixes.filter(p => p !== prefix)
    });
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (bgpConfig.enabled) {
      if (!bgpConfig.asn) newErrors.bgp_asn = 'ASN is required for BGP';
      if (bgpConfig.asn && !/^\d+$/.test(bgpConfig.asn)) newErrors.bgp_asn = 'ASN must be a number';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createColocation({
        ...
        bgp_config: bgpConfig.enabled ? bgpConfig : null,
      });
      
      router.push('/colocation/list');
    } catch (error: any) {
      console.error('Failed to create colocation:', error);
      alert(error.response?.data?.message || 'Failed to create colocation');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-6 max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Add Colocation Space</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Configure a new colocation rack space
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., /29 or 192.168.1.0/29"
                  className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white"
                />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">IPv6 Allocation</label>
                <input
                  type="text"
                  value={formData.ipv6_allocation}
                  onChange={(e) => setFormData({ ...formData, ipv6_allocation: e.target.value })}
                  placeholder="e.g., /64"
                  className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white"
                />
              </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <button
                type="button"
                onClick={() => setShowBgpConfig(!showBgpConfig)}
              ...
              >
                <div className="flex items-center gap-3">
                  <input
                    ...
                    onChange={(e) => {
                      setBgpConfig({ ...bgpConfig, enabled: e.target.checked });
                      if (e.target.checked) setShowBgpConfig(true);
                    }}
                ...
                {bgpConfig.enabled && (
                  showBgpConfig ? <ChevronUp size={20} className="text-gray-400" /> : <ChevronDown size={20} className="text-gray-400" />
                )}
              </button>

              {/* BGP Configuration */}
              {bgpConfig.enabled && showBgpConfig && (
                <div className="mt-4 pl-7 space-y-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={bgpConfig.asn}
                      onChange={(e) => setBgpConfig({ ...bgpConfig, asn: e.target.value })}
                      placeholder="e.g., 64512"
                      className={cn(
                        "w-full px-4 py-2 border rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white",
                        errors.bgp_asn ? "border-red-500" : "border-gray-300 dark:border-gray-600"
                      )}
                    />
                    {errors.bgp_asn && <p className="text-sm text-red-500 mt-1">{errors.bgp_asn}</p>}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <input
                        type="text"
                        value={ipv4PrefixInput}
                        onChange={(e) => setIpv4PrefixInput(e.target.value)}
                        onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addIpv4Prefix())}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {bgpConfig.ipv4_prefixes.map((prefix) => (
                        <span
                          key={prefix}
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => removeIpv4Prefix(prefix)}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <input
                        ...
                        onChange={(e) => setIpv6PrefixInput(e.target.value)}
                        onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addIpv6Prefix())}
                        placeholder="e.g., 2001:db8::/48"
                        className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white"
                      />
                      <button
                        type="button"
                        onClick={addIpv6Prefix}
                        className="px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600"
                      >
                        Add
                      </button>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {bgpConfig.ipv6_prefixes.map((prefix) => (
                        <span
                          key={prefix}
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => removeIpv6Prefix(prefix)}
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <div>
                ...
                <input
                  ...
                  className="w-full md:w-1/2 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white"
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {loading ? 'Creating...' : 'Create Colocation'}
            </button>
            <button
              type="button"
              onClick={() => router.back()}
