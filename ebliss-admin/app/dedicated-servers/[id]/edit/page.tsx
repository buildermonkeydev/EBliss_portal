...
import { getServer, updateServer, DedicatedServer } from '../../../lib/dedicated-servers';
...
const popsData = Array.isArray(popsResponse) 
  ? popsResponse 
  : (popsResponse as any).data || popsResponse;       
   setPops(popsData.filter((pop: Pop) => pop.active));
        

const transformedStorage = (serverData.storage || []).map((storage: any) => ({
  id: storage.id, // Keep the id if it exists
  type: storage.type,
  size_gb: storage.size_gb,
  raid_level: storage.raid_level || 'NONE',
  drive_count: storage.drive_count,
  is_primary: storage.is_primary
}));
...
setFormData({
  ...
  cpu_cores: serverData.cpu_cores || 0,
  cpu_threads: serverData.cpu_threads || 0,
  cpu_speed: serverData.cpu_speed || '',
  ram_gb: serverData.ram_gb || 0,
  ram_type: serverData.ram_type || 'DDR4',
  ram_speed: serverData.ram_speed || '',
  storage: transformedStorage, // Use transformed storage
  os: serverData.os || '',
  os_version: serverData.os_version || '',
  root_password: serverData.root_password || '',
  network_port: serverData.network_port || '1 Gbps',
  bandwidth_tb: serverData.bandwidth_tb || 0,
  ipv4_count: serverData.ipv4_count || 0,
  ipv6_count: serverData.ipv6_count || 0,
  rack_id: serverData.rack_id || '',
  rack_position: serverData.rack_position || '',
  ddos_protection: serverData.ddos_protection || false,
  backup_enabled: serverData.backup_enabled || false,
  monitoring_enabled: serverData.monitoring_enabled ?? true,
  ipmi_ip: serverData.ipmi_ip || '',
  ipmi_user: serverData.ipmi_user || '',
  ipmi_password: serverData.ipmi_password || '',
  kvm_enabled: serverData.kvm_enabled || false,
  kvm_type: serverData.kvm_type || '',
  monthly_price: serverData.monthly_price || 0,
  setup_fee: serverData.setup_fee || 0,
  notes: serverData.notes || '',
  tags: serverData.tags || [],
});
        // Set selected POP
        const matchingPop = popsData.find((p: Pop) => p.name === serverData.datacenter);
        if (matchingPop) {
          setSelectedPop(matchingPop);
          setFormData(prev => ({ ...prev, pop_id: matchingPop.id }));
        }
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    // For existing items (with id), keep as is
    // For new items (no id), remove id field or set as undefined
    const storageForApi = formData.storage.map(({ id, ...rest }) => ({
      ...rest,
    ...
    await updateServer(id, {
      ...formData,
      storage: storageForApi,
    } as any);
    
    router.push(`/dedicated-servers/${id}`);
  } catch (error: any) {
    console.error('Failed to update server:', error);
    const errorMessage = error.response?.data?.message || 'Failed to update server. Please try again.';
    alert(errorMessage);
  } finally {
    setSaving(false);
  }
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Threads</label>
                    <input
                      type="number"
                      min="1"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Clock Speed</label>
                    <input
                      type="text"
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">RAM Type</label>
                    <select
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">RAM Speed</label>
                    <input
                      type="text"
          ...
          {activeTab === 'network' && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Bandwidth (TB/month)</label>
                  <input
                    type="number"
                    min="1"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">IPv4 Addresses</label>
                  <input
                    type="number"
                    min="0"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">IPv6 Addresses</label>
                  <input
                    type="number"
                    min="0"
              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Rack ID</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Rack Position (U)</label>
                    <input
                      type="text"
